<template>
  <div id="dashboard">
    ...
    <div class="form">
      ...
      <input type="submit" name="submit" @click="upload" value="Upload proposal">
    </div>
    ...
    <table>
      ...
      <tr v-for="proposal in proposals" class="proposal">
        ...
        <td><a :href="proposal.url" target="_blank">{{ proposal.url }}</a></td>
...
<script>
import Ballot from '@/js/ballotIPFS'
  ...
  methods: {
    ...
    upload: function () {
      ...
      reader.onloadend = function () {
        ...
        const buffer = Buffer(reader.result) // Convert data into buffer
        ipfs.files.add(buffer, (err, result) => {
          if (err) {
            console.error(err)
            return
          }
          Ballot.createProposal(`https://ipfs.io/ipfs/${result[0].hash}`)
          .then(tx => Ballot.getProposals())
          .then(proposals => { self.proposals = proposals })
          .catch(err => console.log(err))
        })
      ...
      reader.readAsArrayBuffer(proposal.files[0])
    }
  }
}
</script>
